...
> 💡 画像について：現在は各所に**仮のイメージ図（SVGモックアップ）**が入っています（`images/◯◯.svg`）。
> 本番では**あなたが操作中に撮った実スクリーンショット**に差し替えてください。撮る画面の一覧は [撮影リスト.md](撮影リスト.md)。
> **差し替え方**：実スクショ（PNG）を `images/` に置き、この文書内の該当 `![◯◯](images/◯◯.svg)` を `.png` に変える（または仮の `.svg` を消して同名にする）。
> 一括で `.svg`→`.png` に戻すなら、エディタの置換で `images/` の `.svg` を `.png` に置換すればOKです。
...
画面に `Production` と出ているか必ず確認。次の2つを登録：
...
| Vercel デプロイが失敗 | `CONVEX_DEPLOY_KEY` 未登録が多い。ステップ3-3を見直す |
